<template>
    <transition name="slide-fade">
        <div class="the-aside" v-if="showAside" ref="list1">
...
export default {
    ...
    mounted(){
        // let _this = this;
        // document.querySelector('.the-aside').addEventListener('click',function(e){
      ...
      document.addEventListener('click',(e)=>{
        if(this.$refs.list1){
          let isSelf = this.$refs.list1.contains(e.target)
          if(isSelf){
            _this.$store.commit('setShowAside',true);
          }
        }
      })
    },
