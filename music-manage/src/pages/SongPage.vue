    ...
    <el-table size="mini" ref="multipleTable" border style="width:100%" height="680px" :data="data" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="歌曲图片" width="110" align="center">
        <template slot-scope="scope">
          <div class="song-img">
            <img :src="getUrl(scope.row.pic)" style="width:100%" v-image-preview/>
          </div>
          <div class="play" @click="setSongUrl(scope.row.url,scope.row.name)">
            <div v-if="oneSong_playOrNot(scope.row.name)">
              <svg class="icon">
...
export default {
  ...
  methods:{
    oneSong_playOrNot(name) {
      if(this.toggle == name && this.isPlay == true){
        return true
      }
      else {
        return false;
